...
function fileUpload(arg) {
    ...
    jr.event.add(__html_ifr, 'load', _loadCall);
...
fileUpload.prototype.clear = function () {
    //允许重复上传文件
    if (this.repeatSelect && this.file) {
        this.file.outerHTML = this.file.outerHTML;  //清空文件域的值
    }
    ;
};
...
function fileUploadObject(arg, func) {
    ...
    _upload.onUploading = function (json) {
        ...
        try {
            var tipTxt = '上传中...';
            var arr = jr.dom.getsByClass(up, 'button-txt');
            if (arr) {
                jr.each(arr, function (i, e) {
                    e.innerHTML = tipTxt;
                });
            } else {
                up.innerHTML = tipTxt;
    ...
    _upload.onUploadComplete = function (result, data, doc) {
        ...
        try {
            up.innerHTML = _upload.btnText;
        } catch (ex) {
        }

        if (func) {
            func.call(_upload, result, data, doc);
        }
    };
...
(function (r) {
    var obj = {
        upload: function (arg, func) {
            return fileUploadObject(arg, func);
        }
    };
    if (r) {
